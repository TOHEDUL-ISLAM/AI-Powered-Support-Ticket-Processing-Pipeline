// US-2.2 + US-5.2: unit tests for Step 2 worker message parsing, retry scheduling, and permanent failure
...
} from '../../src/services/resolution-step.service';
import { parsePhase2Message, Phase2Worker } from '../../src/workers/phase2.worker';

describe('parsePhase2Message', () => {
  it('accepts a valid UUID ticketId', () => {
    const result = parsePhase2Message(
  ...
  it('rejects invalid JSON', () => {
    const result = parsePhase2Message('not-json');

    expect(result).toEqual({ ok: false, reason: 'invalid_json' });
  });

  it('rejects a missing ticketId', () => {
    const result = parsePhase2Message(JSON.stringify({}));

    expect(result).toEqual({ ok: false, reason: 'invalid_ticket_id' });
  });

  it('rejects a non-UUID ticketId', () => {
    const result = parsePhase2Message(JSON.stringify({ ticketId: 'not-a-uuid' }));

    expect(result).toEqual({ ok: false, reason: 'invalid_ticket_id' });
  });
});

describe('Phase2Worker deletion rules', () => {
  it('deletes malformed messages without calling the processor', async () => {
    ...
    const worker = new Phase2Worker(consumer, service, pino({ level: 'silent' }), {
      waitSeconds: 0,
      maxMessages: 1,
      visibilityTimeout: 1,
    });
  ...
  it('does not delete messages when Step 1 is not ready', async () => {
    ...
    const service = new TriageNotReadyService();
    const worker = new Phase2Worker(consumer, service, pino({ level: 'silent' }), {
      waitSeconds: 0,
      maxMessages: 1,
      visibilityTimeout: 1,
  ...
  it('schedules visibility change and does not delete on failed outcome', async () => {
    ...
    const worker = new Phase2Worker(consumer, service, logger, {
      waitSeconds: 0,
      maxMessages: 1,
      visibilityTimeout: 1,
    });
    ...
    expect(consumer.visibilityChanges[0].receiptHandle).toBe('rh-4');
...
class SuccessfulService implements IResolutionStepService {
  calls = 0;
...
class TriageNotReadyService implements IResolutionStepService {
  calls = 0;
...
class ThrowingService implements IResolutionStepService {
  calls = 0;
...
class FailedOutcomeService implements IResolutionStepService {
  calls = 0;
...
class PermanentlyFailedService implements IResolutionStepService {
  calls = 0;
